...
static void ping(struct netconn *conn) {
    ...
    while(conn_alive){
        ...
        if((curr_time - prev_time)/1000000>1)
        {
          printf("\n time bc : %d",(int)(curr_time - prev_time)/1000000);
          prev_time = curr_time;
          int netconn_res = netconn_write(conn, ping_msg,strlen(ping_msg),NETCONN_COPY);
          if(netconn_res != ERR_OK){
            printf("\n Sendign failed");
          }
        }
